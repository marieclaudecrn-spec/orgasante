...
interface Pilier { label: string; score: number; statut: { label: string; couleur: string } }
interface Alerte { niveau: string; pilier: string; message: string }
interface ScoreData {
  scoreGlobal: number;
  statut: { label: string; couleur: string };
...
export default function Dashboard() {
  ...
  useEffect(() => {
    fetch('/api/score')
      .then(r => r.json())
      .then((d: ScoreData) => { setData(d); setLoading(false); })
    ...
    <main style={{ fontFamily: 'system-ui, sans-serif', maxWidth: 900, margin: '0 auto', padding: '24px 16px', color: '#2C2C2A' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: '#888780', margin: '4px 0 0' }}>Entreprise Exemple inc. — {data.periode}</p>
        </div>
        <span style={{
          background: data.statut.couleur === 'vert' ? '#EAF3DE' : data.statut.couleur === 'orange' ? '#FAEEDA' : '#FCEBEB',
            ...
            return (
              <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                ...
                <div style={{ flex: 1, background: '#F1EFE8', borderRadius: 999, height: 8, overflow: 'hidden' }}>
                  <div style={{ width: `${p.score}%`, height: 8, borderRadius: 999, background: c.color }} />
                </div>
      ...
      <div style={{ background: 'white', border: '0.5px solid #D3D1C7', borderRadius: 12, padding: '16px 20px', marginBottom: 16 }}>
        <p style={{ fontSize: 13, fontWeight: 500, margin: '0 0 10px' }}>
          Alertes IA — {alertes.filter((a: Alerte) => a.niveau === 'rouge').length} critiques
        </p>
        {alertes.map((a: Alerte, i: number) => (
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          {[
            ...
            <div key={s.nom} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: '#F1EFE8', borderRadius: 8 }}>
              <span style={{ width: 8, height: 8, borderRadius: '50%', background: s.connecte ? '#639922' : '#B4B2A9' }} />
